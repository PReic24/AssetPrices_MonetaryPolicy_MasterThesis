%% (Pflueger) This class contains all parameters and methods related to asset prices and their properties
classdef asset_p
    %% List of properties
    properties
        ...
        Irf_saver,                 %   saves temporary impulse response to be examined later, added by Reichert
...
%% Regress FOMC asset returns onto nominal rate changes on FOMC dates
            ...
            crossAsset.corrTipsStock    = correlations(2);
        ...
        function asset = computeFn21(asset_pI, num_set, macro_dyn)
            ...
            % Return Sums
            retnomSum   = zeros(1, 50);
            retrealSum  = zeros(1, 50);
            reteqSum    = zeros(1, 50);
            ...
            while j<=Nsim
                ...
                eps            = A*Q*u;
                % Scale bond prices with long term MP shock, as discussed
                % in Appendix p.27
                uast           = u(4,:);
        ...
        function asset = simulateAllStructuralIRF(asset_pI, macro_dyn, num_set)
            Nsim    = num_set.Nsim;
            Nbonds  = num_set.Nbonds;
            T       = num_set.Tirf;
            N       = num_set.N;
            ...
            % Copy asset prices and set up initial shocks
            asset = asset_pI;
            ...
            % Return Sums:
            reteqSum    = zeros(49, 1);
            retnomSum   = zeros(49, 1);
            retrealSum  = zeros(49, 1);

            const2 =  [1,0,0]*( P- phi*eye(3))* Ainv;
            const3 = ([0,0,1]-[0,1,0]* P)* Ainv;


            nShocks = 4;               % Number of shocks
            asset.initialShockVec = zeros(1, nShocks);  % Initialize a 4x1 zero vector
            
            % Initialize the container for all shocks
            x_all_irfs              = zeros(nShocks, 20);
            shat_all_irfs           = zeros(nShocks, 20);
            c_all_irfs              = zeros(nShocks, 20);
            pi_all_irfs             = zeros(nShocks, 20);
            i_all_irfs              = zeros(nShocks, 20);
            PD_all_irfs             = zeros(nShocks, 20);
            y10nom_all_irfs         = zeros(nShocks, 20);
            y10real_all_irfs        = zeros(nShocks, 20);
            PD_rn_all_irfs          = zeros(nShocks, 20);
            y10nom_rn_all_irfs      = zeros(nShocks, 20);
            y10real_rn_all_irfs     = zeros(nShocks, 20);
            PD_rp_all_irfs          = zeros(nShocks, 20);
            y10nom_rp_all_irfs      = zeros(nShocks, 20);
            y10real_rp_all_irfs     = zeros(nShocks, 20);
            responses_all_irfs      = zeros(nShocks, 20);

            % Return Container
            reteq_all   = zeros(49, nShocks);
            retnom_all  = zeros(49, nShocks);
            retreal_all = zeros(49, nShocks);

            % Return Correlation Container
            corr_eq_nom = zeros(1, nShocks);
            corr_eq_real = zeros(1, nShocks);

            for s = 1:nShocks
                asset.initialShockVec(1, :) = 0;
                asset.initialShockVec(1, s) = 1;
                ...
                while j<=Nsim
                    ...
                    % In percentage units, excess equity return in levels:
                    reteq_lvl          = exp(reteq);
                    ...
                    % Level excess nominal bond returns in percent and
                    % annulaized
                    retnom_ann         = 4*(100*log(Pnomminus(2:end))-100*log(Pnomplus(1:end-1))-rfr_nom(1:end-1)'/4);
                    retnom_ann         = exp(retnom_ann);

                    % Log excess real bond returns in percent but not annualized
                    retreal            = 100*log(Pminus(2:end))-100*log(Pplus(1:end-1))-rfr(1:end-1)'/4;
                    retreal_rn         = 100*log(Pminus_rn(2:end))-100*log(Pplus_rn(1:end-1))-rfr(1:end-1)'/4;
                    
                    % Level excess real bond returns in percent and
                    % annulaized
                    retreal_ann         = 4*(100*log(Pminus(2:end))-100*log(Pplus(1:end-1))-rfr_nom(1:end-1)'/4);
                    retreal_ann         = exp(retreal_ann);
                    ...
                    reteq_lvl          = reteq_lvl(2:end-1);

                    retnom             = retnom(2:end-1);                    
                    retnom_rn          = retnom_rn(2:end-1);

                    retnom_ann         = retnom_ann(2:end-1);

                    retreal            = retreal(2:end-1);
                    retreal_rn         = retreal_rn(2:end-1);

                    retreal_ann        = retreal_ann(2:end-1);
                    ...
                    % Return Sums in annualized percent, levels
                    reteqSum    = reteqSum + reteq_lvl;
                    retnomSum   = retnomSum + retnom_ann;
                    retrealSum  = retrealSum + retreal_ann;
                ...
                x_all_irfs(s, :) = xIrfSum/Nsim;
                c_all_irfs(s, :) = cIrfSum/Nsim; 
                shat_all_irfs(s, :) = shatsimIrfSum/Nsim; 
                pi_all_irfs(s, :)= piIrfSum/Nsim;
                i_all_irfs(s, :) = iIrfSum/Nsim;
                PD_all_irfs(s, :) = PDIrfSum/Nsim;
                y10nom_all_irfs(s, :) = y10nomIrfSum/Nsim;
                y10real_all_irfs(s, :)= y10realIrfSum/Nsim;

                PD_rn_all_irfs(s, :) = PDIrfSum_rn/Nsim;
                y10nom_rn_all_irfs(s, :)= y10nomIrfSum_rn/Nsim;
                y10real_rn_all_irfs(s, :) = y10realIrfSum_rn/Nsim;
            
                PD_rp_all_irfs(s, :) = PDIrfSum_rp/Nsim;
                y10nom_rp_all_irfs(s, :) = y10nomIrfSum_rp/Nsim;
                y10real_rp_all_irfs(s, :) = y10realIrfSum_rp/Nsim;
                %responses_all_irfs(s, :) = responsesSum/Nsim;

                % Return Simulation averages in logs
                reteq_all(:,s)  = reteqSum/Nsim;
                retnom_all(:,s) = retnomSum/Nsim;
                retreal_all(:,s)= retrealSum/Nsim;

                % Return Correlations
                corr_eq_nom(1,s)    = corr(reteq_all(:,s), retnom_all(:,s));
                corr_eq_real(1,s)   = corr(reteq_all(:,s), retreal_all(:,s));
            end
            

            Irftemp.x           = x_all_irfs;
            Irftemp.c           = c_all_irfs; 
            Irftemp.pi          = pi_all_irfs;
            Irftemp.i           = i_all_irfs;
            Irftemp.PD          = PD_all_irfs;
            Irftemp.shatsim     = shat_all_irfs;
            Irftemp.y10nom      = y10nom_all_irfs;
            Irftemp.y10real     = y10real_all_irfs;
            Irftemp.PD_rn       = PD_rn_all_irfs;
            Irftemp.y10nom_rn   = y10nom_rn_all_irfs;
            Irftemp.y10real_rn  = y10real_rn_all_irfs;
            Irftemp.PD_rp       = PD_rp_all_irfs;
            Irftemp.y10nom_rp   = y10nom_rp_all_irfs;
            Irftemp.y10real_rp  = y10real_rp_all_irfs;
            %Irftemp.responses   = responsesObj;
            
            % Return Correlations betw. logs
            Irftemp.stock_nombond_corr  = corr_eq_nom;
            Irftemp.stock_realbond_corr = corr_eq_real;
            asset.Irftemp    =   Irftemp;
            return
        end

%% Run Simulation without FOMC Stuff
        function asset = simulateMoments_wthout_FOMC(asset_pI, num_set, macro_dyn)
