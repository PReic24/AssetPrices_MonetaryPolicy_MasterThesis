...
%-------------------------------------------------------------------------%
clear all
%function minimum_working_example_NK(gamma_x, gamma_pi)
    %clear all
    %if nargin < 2
    %   gamma_x = 0.5 / 4;  % Default MP Response to output (annualized percent)
    %   gamma_pi = 1.5;     % Default MP Response to inflation
    %end
...
macro1.gamma_x          =   0.5/4;        % if function = gamma_x, MP Response to output (annualized percent)
macro1.gamma_pi         =   1.5;       % if function = gamma_pi, MP Response to inflation
...
asset =  asset.simulateAllStructuralIRF(macro1,num1);
toc;
% Save third structural IRF
 asset.Irf3=asset.Irftemp;

% Test moment generator without FOMC stuff:
% disp("Simulate Moments without FOMC Stuff")
%asset_2 = asset.simulateMoments_wthout_FOMC(num1,macro1);

% Plotting function for Figure 3
plot_StructuralIRF_STMP(asset);
%plot_asset_irfs(asset);
%plot_macro_irfs(asset);
%plot_shat_irfs(asset);
%return
%end